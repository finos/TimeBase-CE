...
@Function("MIN")
public class MinVarchar extends VarcharToVarcharStatefulFunctionBase {

    @Compute
    @Override
    public void compute(@BuiltInTimestampMs long timestamp, CharSequence v) {
        if (TimebaseTypes.isNull(v)) {
            return;
        }
        if (TimebaseTypes.isNull(value) || Util.compare(v, value, false) < 0) {
            sb.setLength(0);
            sb.append(v);
            value = sb;
        }
