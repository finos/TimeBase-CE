...
abstract class QQLPostProcessingPatterns {
    ...
    public static TimestampLimits extractTimestampLimits(List<CompiledExpression> condition) {
        ...
        for (int ii = 0; ii < condition.size(); ) {
            ...
            left = extractSelector(left);
            right = extractSelector(right);
            if (left instanceof TimestampSelector && isStatic(right)) {
                if (range == null) {
                    range = new TimestampLimits();
                }

                TimestampSelector selector = (TimestampSelector) left;
                matched = updateRange(range, selector, relation, right, false);
            } else if (right instanceof TimestampSelector && isStatic(left)) {
                if (range == null) {
                    range = new TimestampLimits();
                }

                TimestampSelector selector = (TimestampSelector) right;
                matched = updateRange(range, selector, relation, left, true);
            }
    ...
    private static CompiledExpression<?> extractSelector(CompiledExpression<?> c) {
        if (c instanceof SimpleFunction && ((SimpleFunction) c).code == SimpleFunctionCode.MS_TO_NS &&
            ((SimpleFunction) c).args[0] instanceof TimestampSelector) {

            return  ((SimpleFunction) c).args[0];
        }

        return c;
    }

    private static boolean updateRange(TimestampLimits range,
        ...
        boolean matched = false;
        boolean isSelectorNs = selector.hasNanosecondPrecision();
        if (arg instanceof CompiledConstant) {
            CompiledConstant constant = (CompiledConstant) arg;
            boolean isConstNs = isSelectorNs;
            if (DataTypeHelper.isTimestampNs(constant.type)) {
                isConstNs = true;
            }
            ...
            if (isSelectorNs) {
                ...
                range.update((ParamAccess) arg, relation, timestampOnRight, false);
                matched = true;
            }
        }
