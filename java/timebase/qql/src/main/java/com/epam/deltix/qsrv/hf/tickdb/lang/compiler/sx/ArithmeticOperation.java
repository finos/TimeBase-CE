...
public class ArithmeticOperation extends BinaryExpression {
    ...
    public static void validateArgs(ArithmeticExpression expression, CompiledExpression<?> left, CompiledExpression<?> right) throws IllegalTypeCombinationException {
        if (expression.function == ArithmeticFunction.MOD) {
            ...
            if (isVarcharConcat(expression, left, right)) {
                return;
            }
    ...
    public static boolean isVarcharConcat(ArithmeticExpression expression, CompiledExpression<?> left, CompiledExpression<?> right) {
        return expression.function == ArithmeticFunction.ADD && left.type instanceof VarcharDataType && right.type instanceof VarcharDataType;
    }
