...
class QueryStatusMessageProvider {
    ...
    QueryStatusMessageProvider(RecordClassDescriptor[] outputTypes) {
        for (int i = 0; i < outputTypes.length; ++i) {
            if (QueryStatusMessage.CLASS_NAME.equals(outputTypes[i].getName())) {
                queryStatusMessageEncoder = COMPILED_FACTORY.createFixedBoundEncoder(
    ...
    RawMessage prepareQueryStatusMessage(long nanoTime, QueryStatus status, String cause) {
        ...
        queryStatusMessage.setNanoTime(nanoTime);
