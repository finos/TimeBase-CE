...
public class StreamCreator extends LoggingActivityLauncher {
    ...
    public StreamCreator (DXTickDB db, String key, StreamOptions options, boolean ifNotExists) {
        this.db = db;
        this.key = key;
        ...
        return (
            new LoggingActivity () {
                ...
                protected void      run (ReadableValue[] params) {
                    if (ifNotExists && db.getStream(key) != null) {
                        log (SUCCESS, ErrorLevel.INFO, "Stream exists", key);
                        return;
                    }
