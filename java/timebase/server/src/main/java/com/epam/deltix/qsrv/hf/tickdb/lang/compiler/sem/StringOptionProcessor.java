 ...
 *
 */
public abstract class StringOptionProcessor<T> extends OptionProcessor<T> {
    public StringOptionProcessor(String key) {
        super(key, StandardTypes.CLEAN_VARCHAR);
    }

    protected abstract void set(T target, String value);

    protected abstract String get(T source);
    ...
    public final void process(OptionElement option, CompiledConstant value, T target) {
        ...
        set(target, value.getString());
    }

    @Override
    protected void printValue(T source, StringBuilder out) {
        out.append("'").append(get(source)).append("'");
    }
}
